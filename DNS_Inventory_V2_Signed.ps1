...
Function OutputLookupZoneDetails
{
	...
	ForEach($Detail in $ZoneDetails)
	{
		...
		ElseIf($Detail.RecordType -eq "RRSIG")
		{
			...
			$DetailData = "[$($Detail.RecordData.TypeCovered)][Inception(UTC): $($InceptionDate) $($InceptionTime)][Expiration(UTC): $($ExpirationDate) $($ExpirationTime)][$($Detail.RecordData.NameSigner)][$($Crypto)][$($Detail.RecordDa
ta.LabelCount)][$($Detail.RecordData.KeyTag)]"
		}
